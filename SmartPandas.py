...
class FeatureDivision(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        return pd.DataFrame(
            X[self.columna_dividendo] / X[self.columna_divisor],
...
class CorrSpearman(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        return abs(
            X.corr(method="spearman")[self.target].drop(index=self.target).sort_values()
        )
...
class BuscarIndex:
    """Es para la notebook 1 del tp_final"""
...
def evaluate_model(
    ...
    y_test,
    gridSearch_params,
    gridSearch_bagging_params=False,
    ...
    grid_search_CV_KFold_model = GridSearchCV(
        model_instance, gridSearch_params, n_jobs=-1, cv=cv_KFold
    )
    grid_search_CV_KFold_model.fit(X_train, y_train)
    scores_KFold = cross_val_score(
        model_instance, X_train, y_train, cv=cv_KFold, n_jobs=-1
    )
    # metelo en un dic
    ...
    params_grid_search_CV_KFold_model = (
        grid_search_CV_KFold_model.best_estimator_.get_params()
    )

    score_grid_search_CV_KFold_model_test = grid_search_CV_KFold_model.score(
        X_test, y_test
    )
    predictions_grid_search_CV_KFold_model = grid_search_CV_KFold_model.predict(X_test)
    confusion_matrix_grid_search_CV_KFold_model = metrics.confusion_matrix(
        y_test, predictions_grid_search_CV_KFold_model
    )

    ###################################################

    # gridSearch Stratified KFold:
    cv_Stratified_KFold = StratifiedKFold(
        n_splits=n_split, shuffle=True, random_state=random_state
    )
    grid_search_CV_Stratified_KFold_model = GridSearchCV(
        model_instance, gridSearch_params, n_jobs=-1, cv=cv_Stratified_KFold
    )
    grid_search_CV_Stratified_KFold_model.fit(X_train, y_train)
    scores_Stratified_KFold = cross_val_score(
        model_instance, X_train, y_train, cv=cv_Stratified_KFold, n_jobs=-1
    )
    ...
    score_grid_search_CV_Stratified_KFold_model = (
        grid_search_CV_Stratified_KFold_model.best_score_
    )
    params_grid_search_CV_Stratified_KFold_model = (
        grid_search_CV_Stratified_KFold_model.best_estimator_.get_params()
    )

    score_grid_search_CV_Stratified_KFold_model_test = (
        grid_search_CV_Stratified_KFold_model.score(X_test, y_test)
    )
    predictions_grid_search_CV_Stratified_KFold_model = (
        grid_search_CV_Stratified_KFold_model.predict(X_test)
    )
    confusion_matrix_grid_search_CV_Stratified_KFold_model = metrics.confusion_matrix(
        y_test, predictions_grid_search_CV_Stratified_KFold_model
    )
    ...
    if gridSearch_bagging_params:
        ...
        confusion_matrix_bagging_model_default = metrics.confusion_matrix(
            y_test, predictions_bagging_model_default
        )

        ###################################################

        # bagging Stratified KFold cross validation usando de base el mejor modelo de gridsearch estratificado
        base_estimator_stratified_grid_search = (
            grid_search_CV_Stratified_KFold_model.best_estimator_
        )
        cv_Stratified_KFold = StratifiedKFold(
            n_splits=n_split, shuffle=True, random_state=random_state
        )
        grid_search_bagging_model = GridSearchCV(
            BaggingClassifier(base_estimator=base_estimator_stratified_grid_search),
            gridSearch_bagging_params,
            n_jobs=-1,
            cv=cv_Stratified_KFold,
        )

        grid_search_bagging_model.fit(X_train, y_train)

        score_grid_search_bagging_model_test = grid_search_bagging_model.score(
            X_test, y_test
        )
        predictions_grid_search_bagging_model = grid_search_bagging_model.predict(
            X_test
        )
        confusion_matrix_grid_search_bagging_model = metrics.confusion_matrix(
            y_test, predictions_grid_search_bagging_model
        )
        scores_bagging_Stratified_KFold = cross_val_score(
            BaggingClassifier(base_estimator=base_estimator_stratified_grid_search),
            X_train,
            y_train,
            cv=cv_Stratified_KFold,
        ...
        mean_score_bagging_grid_search_CV_Stratified_KFold_model = (
            scores_bagging_Stratified_KFold.mean()
        )
        std_score_bagging_grid_search_CV_Stratified_KFold_model = (
            scores_bagging_Stratified_KFold.std()
        )

        best_score_bagging_grid_search_CV_Stratified_KFold_model = (
            grid_search_bagging_model.best_score_
        )
        best_params_bagging_grid_search_CV_Stratified_KFold_model = (
            grid_search_bagging_model.best_estimator_.get_params()
        )
    ...
    result_params = {
        ...
        "cv_stratified_kfold_best_estimator_params": [
            params_grid_search_CV_Stratified_KFold_model
        ],
        "bagging_cv_stratified_kfold_best_estimator_params": [
            best_params_bagging_grid_search_CV_Stratified_KFold_model
        ],
    }
...
def featureImportance(model, X_train):
    ...
    importance.sort_values(
        by="feature_importance", axis=0, ascending=False, inplace=True
    )
    return importance
